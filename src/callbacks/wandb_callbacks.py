...
class LogValPredictionsSegmentation(Callback):
    ...
    def __init__(self, num_samples: int = 8, every_n_epochs: int = 10, log_scatter: bool = False):
        super().__init__()
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        if self.ready and (trainer.current_epoch + 1) % self.every_n_epochs == 0:
            ...
            # inputs could have a time dimension and shape of (num_samples, num_vars, t, height, width)
            # if t > 1, take the last timestep
            if len(inputs.shape)==5 and inputs.shape[2] > 1:
                inputs = inputs[:, :, -1, :, :].squeeze()
